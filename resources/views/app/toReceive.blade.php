                ...
                <div class="col-md-6 mb-2">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <p><span class="fw-bold">Serviço realizado em:</span>
                                {{ date_format($toReceive->created_at, 'd-m-y h:i:s') }}
                            </p>
                            @if (!$toReceive->status)
                                <x-form route="toReceive.update" key="toReceive" id="{{ $toReceive->id }}" method="POST"
                                    http-verb="PUT">
                                ...
                                <a class="btn btn-success w-100 my-2" href="tel:{{ $toReceive->customer->phone }}"
                                    title="Ligar para o cliente">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25"
                                        fill="currentColor" class="bi bi-telephone-outbound me-2" viewBox="0 0 16 16">
                                        <path
                            ...
                            @if ($toReceive->customer->deleted_at === null)
                                <x-link route="customer.show" key="customer" id="{{ $toReceive->customer->id }}"
                            ...
                            <x-form route="toReceive.destroy" key="toReceive" id="{{ $toReceive->id }}" method="POST"
                                http-verb="DELETE">
            ...
            <ul class="pagination">
                ...
                @for ($i = 1; $i <= $toReceives->lastPage(); $i++)
                    <li class="page-item {{ $toReceives->currentPage() == $i ? 'active' : '' }}">
                        <a class="page-link bg-teal text-light" href="{{ $toReceives->url($i) }}">{{ $i }}</a>
                    </li>
                @endfor
                <li class="page-item">
